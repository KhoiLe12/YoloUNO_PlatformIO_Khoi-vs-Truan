...
void handleSetMode(){
  ...
  if (m == "ML") {
    servo_set_mode(MODE_ML);
    // In ML mode, show solid LED to indicate AI control is active
    set_led_mode(LED_ON);
  } else {
    servo_set_mode(MODE_MANUAL);
    // In Manual mode, turn the LED off. (No more temperature-driven LED.)
